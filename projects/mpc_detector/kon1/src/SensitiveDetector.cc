...
G4bool SensitiveDetector::ProcessHits(G4Step *step, G4TouchableHistory *hist)
{
      ...
      double energy_kin = step->GetPreStepPoint()->GetPosition().y();
      ...
      if(volume_sens && particle_code == 11 && step->GetTrack()->GetParentID() > 0) {
          G4cout << "Coordinate of electron = " << G4BestUnit(energy_kin, "Length") << " in sensitive volume: " << volume_sens << G4endl;
