...
G4bool SensitiveDetector::ProcessHits(G4Step *step, G4TouchableHistory *hist)
{
      ...
      if(volume_sens && particle_code == 13) {
          G4cout << "Coordinate of " << step->GetTrack()->GetParticleDefinition()->GetParticleName() << " = " << G4BestUnit(energy_kin, "Energy") << " in sensitive volume : " << volume_sens << G4endl;
